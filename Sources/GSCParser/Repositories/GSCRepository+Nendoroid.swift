...
public extension GSCRepository {
    ...
    func parseNendoroidList(option: [ListParseOption] = [.announce, .range, .release]) async -> Set<Nendoroid> {
    ...
    func parseNendoroidList(option: ListParseOption) async -> Set<Nendoroid> {
        await parseNendoroidList(option: [option])
    }
    ...
    private func parseBaesNendoroid(locale: LanguageCode, element: Element) throws -> Nendoroid? {
        ...
        guard
            let gscCode = try String(
                element.attr("href")
                    .replacingOccurrences(of: "https://www.goodsmile.info/\(locale)/product", with: "").split(separator: "/")
                    .first!)
